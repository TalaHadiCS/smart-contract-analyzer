...
/* =========================================
   RISK LOGIC
   Prefers the contract's actual stored risk
   label (set by the AI analysis), so this page
   agrees with My Contracts whenever that label
   is present. If it's ever missing, falls back
   to deriving Low/Medium/High from the score
   instead of showing "Pending" - the analysis
   already ran, so the report should always show
   a real result.
========================================= */

function getRiskInfo(risk, score) {
  ...
  // No usable risk label from the AI - derive one from the
  // score so the report always shows a real result.
...
const riskInfo = getRiskInfo(reportData.risk, reportData.overallScore)
